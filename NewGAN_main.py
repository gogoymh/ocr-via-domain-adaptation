...
parser.add_argument("--exp", type=int, default=21, help="number of experiment")
...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(EMNIST):
        ...
        for refine in range(100):
            ...
            # SimGAN
            D_ref_y = D_ref_y.view(-1,2)
            gt_y = Variable(torch.ones(D_ref_y.size(0))).long().cuda()
            g_loss_adv = local_adversarial_loss(D_ref_y, gt_y)
            
            g_loss_reg = self_regularization_loss(ref_imgs, syn_imgs)
            g_loss_reg_scale = torch.mul(g_loss_reg, opt.delta)
            
            g_loss_class = g_loss_adv + g_loss_reg_scale
            
            g_loss = g_loss_class #0.001*g_loss_began +  #  #
        ...
        # SimGAN
        D_real_y = D_real_y.view(-1,2)
        gt_y_real = Variable(torch.ones(D_real_y.size(0))).long().cuda()
        
        d_loss_real2 = local_adversarial_loss(D_real_y, gt_y_real)
        
        D_ref_y = D_ref_y.view(-1,2)
        gt_y_ref = Variable(torch.zeros(D_ref_y.size(0))).long().cuda()        
        d_loss_fake2 = local_adversarial_loss(D_ref_y, gt_y_ref)
        
        d_loss_class = (d_loss_real2 + d_loss_fake2)/2
        
        d_loss = d_loss_class #0.001*d_loss_began +  # #
