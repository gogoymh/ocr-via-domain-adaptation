...
parser.add_argument("--exp", type=int, default=5, help="number of experiment")
...
parser.add_argument("--g_ratio", type=float, default=1, help="Ratio for generator")
parser.add_argument("--d_ratio", type=float, default=1, help="Ratio for discriminator")
...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(EMNIST):
        ...
        # Refine
        g_loss_adv = torch.mean(torch.abs(discriminator(aux_img) - aux_img))
        ...
        # Sum Up
        g_loss = opt.g_ratio*g_loss_aux + g_loss_ref
        ...
        # Sum up
        d_loss = opt.d_ratio*d_loss_aux + d_loss_ref
        ...
        print(
            ...
            % (epoch, opt.n_epochs, i, len(EMNIST), d_loss_aux.item(), g_loss_aux.item(), M_aux, k_aux)
        )
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(aux_img.data[:25], "images/exp%d/%d_aux.png" % (opt.exp, batches_done), nrow=5, normalize=True)
            save_image(ref_img.data[:25], "images/exp%d/%d_ref.png" % (opt.exp, batches_done), nrow=5, normalize=True)
            
            save_image(d_real_aux.data[:25], "images/exp%d/%d_aux_d.png" % (opt.exp, batches_done), nrow=5, normalize=True)
            save_image(d_real_ref.data[:25], "images/exp%d/%d_ref_d.png" % (opt.exp, batches_done), nrow=5, normalize=True)
