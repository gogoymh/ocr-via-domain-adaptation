...
for epoch in range(opt.n_epochs):
    for i, imgs in enumerate(EMNIST):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(aux_img.data[:25], "images/test_emnist2/%d_aux.png" % batches_done, nrow=5, normalize=True)
            #save_image(ref_img.data[:25], "images/test/%d_ref.png" % batches_done, nrow=5, normalize=True)
            
            save_image(d_real_aux.data[:25], "images/test_emnist2/%d_aux_d.png" % batches_done, nrow=5, normalize=True)
            #save_image(d_real_ref.data[:25], "images/test/%d_ref_d.png" % batches_done, nrow=5, normalize=True)
            
            save_image(syn_imgs.data[:25], "images/test_emnist2/%d_aux_gt.png" % batches_done, nrow=5, normalize=True)
