...
for epoch in range(opt.n_epochs):
    for i, (imgs, _) in enumerate(EMNIST):
        ...
        if batches_done % opt.sample_interval == 0:
            save_image(aux_img.data[:25], "images/%d_aux.png" % batches_done, nrow=5, normalize=True)
            save_image(ref_img.data[:25], "images/%d_ref.png" % batches_done, nrow=5, normalize=True)
